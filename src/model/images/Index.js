...
class Index {
    constructor() {
        ...
        this.model = db.model('images', this.schema);
    ...
    async base64(data) {
        ...
        if (localimg['err_code'] == 200) {
            ...
            return new this.model(data).save().then(function(result) {
                return {err_code: 200, err_msg: '保存成功', data: result}

            }, function(err) {
                return {err_code: 103, err_msg: '保存失败'}
            })
        } else {
            return new Promise(function(resove, reject) {
                resove({err_code: 103, err_msg: '保存失败'})
            })
        }
    ...
    //获取图片信息
    sizeImg(data) {
        return sizeOf(webconfig.source + '/images/' + data).then(dimensions => {
            return dimensions;
